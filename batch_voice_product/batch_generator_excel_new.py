    ...
    def process_excel(self):
        ...
        try:
            ...
            for idx, row in df.iterrows():
                ...
                try:
                    # ==== 新增音色选择逻辑 ====
                    self.client.predict(api_name="/change_choices")

                    # ==== 新API调用方式 ====
                    voice_path = Path(input_root) / row["参考音频"]  # 使用Path处理路径
                    result = self.client.predict(
                        name=self.config["generation"].get("voice_name", "使用参考音频"),
                        voice=handle_file(str(voice_path)),  # 处理音频路径
                        text=row["合成文本"],
                        speed=self.config["generation"].get("speed", 1),
                        api_name="/infer"
                    )
